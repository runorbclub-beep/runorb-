...
class MatchsEventTypeController extends Controller
{
...
//                    ->orwhere("store.store_id","=",$_search);
        ...
        $_arrOfMatchEventTypeQuery = $_arrOfMatchEventTypeQuery->select(
            "matchs_event_type_id","match_events_type_title","match_events_type_title_en","match_events_distance_value","index"
        )->orderBy("index","ASC");
    ...
    public function postMatchEventTypeAdd(Request $request){
        ...
        $_arrOfMatchsEventsTypeData = array(
            "match_events_type_title"=>$_data["match_events_type_title"],
            "match_events_type_title_en"=>$_data["match_events_type_title_en"],
            "index"=>$_data["index"],
            "status"=>1,
    ...
    public function postMatchEventTypeDelete(Request $request){
        ...
        $_arrOfMatch = SysMatch::where([
            "status"=>1,
            "matchs_event_type_id"=>$_data["matchs_event_type_id"]
        ])->select("sys_match_id")->get();
    ...
    /**
     * @author pengjl
     * @time 2021/5/26 15:09
     * @abstract _修改比赛项目序列
     */
    public function postMatchEventTypeIndexUpdate(Request $request){
        $_token_key = "admin_user_token:".$request->header("token");
        $_data = $request->input();
