...
//        本次运动作为用户上次运动数据
        ...
        if (empty($_user_token)){//修复没拿到token
            $_usr_user = UsrUser::with('user_achievement_one')->where('user_id',$_user_play['user_id'])->first();
            if (!$_usr_user){
                $_usr_user['achievement'] = $_usr_user['user_achievement_one'];
                unset($_usr_user['user_achievement_one']);
            }
        }
...
//        获取用户记录
        ...
        $_arrOfNewAchivementUpdateData = array();
...
//        持续时间记录
        if ($_user_play["duration"] > $_usr_user["achievement"]["duration"]) {
            ...
            $_arrOfNewAchivementUpdateData["duration"] = $_user_play["duration"];
        }
...
//        最高转速记录
        if ($_user_play["speed_max"] > $_usr_user["achievement"]["speed_max"]) {
            ...
            UserAchievement::where(["user_id" => $_usr_user["user_id"]])->update([
                ...
                "speed_max_time" => $_user_play["stop_time"]
            ]);
            $_arrOfNewAchivementUpdateData["speed_max"] = $_user_play["speed_max"];
        }

////       摇跑马拉松
//        if (!isset($_usr_user["achievement"]["marathon"])) {
//            $_usr_user["achievement"]["marathon"] = UserAchievement::where(["user_id" => $_usr_user["user_id"]])->value('marathon');
//        }
//
//        if ($_usr_user["achievement"]["marathon"] == 0 || $_usr_user["achievement"]["marathon"] == null || $_usr_user["achievement"]["marathon"] > $_user_play["marathon"]) {
//            //异常过滤
//            $_abnormal_index = StaticDataController::$_abnormal_index;
//            $_user_play["marathon"] = $_user_play["marathon"] >= $_abnormal_index['marathon'] ? $_user_play["marathon"] : 0;
//
//            $_usr_user["achievement"]["marathon"] = $_user_play["marathon"];
//            UserAchievement::where(["user_id" => $_usr_user["user_id"]])->update(["marathon" => $_user_play["marathon"], "marathon_time" => $_user_play["stop_time"]]);
//        }
...
//        最高圈数记录
        if ($_user_play["circle_count"] > $_usr_user["achievement"]["circle_count"]) {
            ...
            $_arrOfNewAchivementUpdateData["circle_count"] = $_user_play["circle_count"];
        }
...
//        耐力记录
        if ($_user_play["endurance_max"] > $_usr_user["achievement"]["endurance_max"]) {
            ...
            $_arrOfNewAchivementUpdateData["endurance_max"] = $_user_play["endurance_max"];
        }

//        更新用户最高记录
        $_usr_user["achievement"]["play_count"] = $_usr_user["achievement"]["play_count"] + 1;
        $_arrOfNewAchivementUpdateData["play_count"] = $_usr_user["achievement"]["play_count"] + 1;

        Redis::hset("usr_user", $_user_token, json_encode($_usr_user));

//        修改个人记录
        UserAchievement::where(["user_id" => $_usr_user["user_id"]])->update($_arrOfNewAchivementUpdateData);
...
//        dd($_arrOfUserMedal->toArray(),$_arrOfUserMedalKey);
...
//        获取用户信息
        ...
        $_arrOfUserAchievement = UserAchievement::where([
            "user_id" => $_usr_user["user_id"],
            "status" => 1
        ])->select("user_achievement_id", "runball_exponent", "duration", "speed_max", "circle_count", "endurance_max", "play_count", "distance_max", "thrmin", "exponent_molecular", "half_marathon", "marathon")->get();
...
//        创建数据
        if (count($_arrOfUserAchievement) == 0) {
            ...
            $_redis_user_achievement = array(
                ...
                "marathon" => $_arrOfUserAchievement[0]["marathon"],
                "exponent_molecular" => $_arrOfUserAchievement[0]["exponent_molecular"],
            );
        }
