...
class UserPlayFunction
{
     ...
     * @return array
     */
    public static function StopPlayFormatData(array $_user_play,array $_last_play,int $_start_time,int $_stop_time,int $_interval){

//        服务端监控到的持续时间
        $_sys_duration = $_user_play["sys_stop_time"] - $_user_play["sys_start_time"];

//        客户端监控到的持续时间
        $_client_duration = $_stop_time - $_start_time;

//        误差数据
        $_sub_duration = $_sys_duration - $_client_duration;
...
//        if
        if ($_sub_duration > 10){
            $_end_duration = $_sys_duration;
        }else {
            $_end_duration = $_client_duration;
        }
...
//        获取区间数组
        ...
        $_max_speed = 0;
        $_circle_count = count($_circle_detail)==0?0:$_circle_detail[count($_circle_detail)-1];
        $_arr_endurance_max = [];
...
//        $_secand = count($_circle_detail)*$_interval/1000;
//        $_user_play["duration"] = $_secand>0?round($_secand-1,3):0;
        ...
        $_user_play["duration"] = $_end_duration;
...
//        距离，米
     ...
     * @return array
     */
    public static function getLastDate(String $_start_date,String $_type,$_language){
        $_arrOfDate = array();
            ...
            switch ($_type){
                ...
                case "month":
                    for ($_i=0;$_i<12;$_i++){
                        ...
                        $_format_title = date("F",$_start_time);

                        $_format_title = substr($_format_title,0,3);
...
//     * @abstract _运动结束，赛事排名更新
//     */
//    public static function matchPlayStop($_sys_match_id,$_matchs_stage_id,$_user_group_id,$_user_id){
//
//        Redis::select(14);
//        $_key = $_sys_match_id.":".$_matchs_stage_id.":".$_user_group_id;
//        $_matchsPlayList = Redis::lrange($_key,0,-1);
//
//        $_duration = 0;
...
//        $_arrOfMatchsUserGrade = MatchsUserGrade::where([
//            "matchs_stage_id"=>$_matchs_stage_id,
//        ])->select("matchs_user_grade_id","match_grade","match_ranking")->orderBy("match_grade","DESC")->get();
//
