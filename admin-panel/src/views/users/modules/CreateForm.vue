  ...
  >
    <a-spin :spinning="loading">
      <a-form :form="form" v-bind="formLayout">
        ...
        <a-form-item v-show="false" label="用户团队ID">
          <a-input v-decorator="['user_group_id', { initialValue: 0 }]" disabled />
        </a-form-item>
        ...
        <a-form-item label="团队性质" has-feedback>
          <a-select
            v-decorator="['is_official', { rules: [{ required: true, message: '请选择团队性质!' }] }]"
            placeholder="请选择团队性质"
          >
            <a-select-option :key="item.value" v-for="item in groupProperty" :value="item.value">
              {{ item.value }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="团队描述">
          <a-input
            v-decorator="['official_description', { rules: [{ required: true, message: '请输入团队描述!' }] }]"
          />
        </a-form-item>
        <a-form-item label="团队logo" extra="">
          <a-upload
            v-decorator="[
              'group_logo',
              {
...
// 表单字段
const fields = ['user_group_id', 'group_title', 'group_num', 'is_official', 'official_description', 'group_logo']
const groupProperty = [
  {
    value: '官方'
  },
...
export default {
  ...
  created() {
    ...
    this.$watch('model', () => {
      ...
      if (this.model !== null) {
        this.model.is_official = this.model.is_official === '1' ? '官方' : '非官方'
        this.isEdit = true
        const groupLogo = this.model.group_logo
        const groupLogoName = this.model.group_logo.split('/')[this.model.group_logo.split('/').length - 1]
        this.$set(this.fileList, 0, {
          uid: '-1',
          name: groupLogoName,
          status: 'done',
          url: this.imgURL + groupLogo
        })
      } else {
        this.fileList = []
        this.isEdit = false
